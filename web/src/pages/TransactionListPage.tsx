...
export default function TransactionListPage() {
  ...
  const prefs = useTablePrefs({ persistKey: "list:transactions" });
  const bulkDelete = useBulkDelete<Transaction>(TransactionBackend.deleteTransaction, list.refetch);
  ...
  const columns: Column<Transaction>[] = [
    ...
    { key: "tag", title: t("transactions.field.tag"), sortable: true, filterable: true, width: "120px" },
    {
      key: "user", title: t("transactions.field.user"), sortable: true, filterable: true, width: "120px",
    ...
    {
      key: "application", title: t("transactions.field.application"), sortable: true, filterable: true, width: "150px",
    ...
    {
      key: "domain", title: t("transactions.field.domain"), sortable: true, filterable: true, width: "200px",
    ...
    { key: "category", title: t("transactions.field.category"), sortable: true, filterable: true, width: "120px" },
    { key: "type", title: t("transactions.field.type"), sortable: true, filterable: true, width: "120px" },
    {
      key: "provider", title: t("transactions.field.provider"), sortable: true, filterable: true, width: "150px",
      render: (_, r) => r.provider ? <span className="text-[12px]">{r.provider}</span> : <span className="text-text-muted">{"\u2014"}</span>,
    },
    { key: "state", title: t("col.state"), sortable: true, filterable: true, width: "100px" },
    {
      key: "amount", title: t("transactions.field.amount"), sortable: true, filterable: true, width: "140px",
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[13px] text-text-muted mt-0.5">{t("transactions.subtitle")}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleAdd} className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2 text-[13px] font-semibold text-white hover:bg-accent-hover transition-colors"><Plus size={15} /> {t("transactions.add")}</button>
        </div>
      </div>
      <DataTable
        columns={columns}
