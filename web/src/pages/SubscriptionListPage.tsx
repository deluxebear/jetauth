...
export default function SubscriptionListPage() {
  ...
  const columns: Column<Subscription>[] = [
    ...
    { key: "period", title: t("subscriptions.field.period"), filterable: true, width: "120px" },
    {
      key: "startTime", title: t("subscriptions.field.startTime"), filterable: true, width: "140px",
      render: (_, r) => <span className="text-[12px] text-text-muted font-mono">{r.startTime ? new Date(r.startTime).toLocaleDateString() : "\u2014"}</span>,
    },
    {
      key: "endTime", title: t("subscriptions.field.endTime"), filterable: true, width: "140px",
      render: (_, r) => <span className="text-[12px] text-text-muted font-mono">{r.endTime ? new Date(r.endTime).toLocaleDateString() : "\u2014"}</span>,
    },
    {
      key: "plan", title: t("subscriptions.field.plan"), filterable: true, width: "140px",
    ...
    {
      key: "user", title: t("subscriptions.field.user"), filterable: true, width: "140px",
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight">{t("subscriptions.title")}</h1>
          <p className="text-[13px] text-text-muted mt-0.5">{t("subscriptions.subtitle")}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleAdd} className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2 text-[13px] font-semibold text-white hover:bg-accent-hover transition-colors"><Plus size={15} /> {t("subscriptions.add")}</button>
        </div>
      </div>
      <DataTable
        columns={columns}
