...
export default function AgentListPage() {
  ...
  const columns: Column<Agent>[] = [
    ...
    {
      key: "url", title: t("agents.field.listeningUrl"), sortable: true, filterable: true,
      render: (_, r) => r.url ? (
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight">{t("agents.title")}</h1>
          <p className="text-[13px] text-text-muted mt-0.5">{t("agents.subtitle")}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleAdd} className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2 text-[13px] font-semibold text-white hover:bg-accent-hover transition-colors"><Plus size={15} /> {t("agents.add")}</button>
        </div>
      </div>
      <DataTable
        columns={columns}
