...
export default function RoleEditPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <StickyEditHeader
        title={`${isAddMode ? t("common.add") : t("common.edit")} ${t("roles.title")}`}
        subtitle={`${owner}/${name}`}
        onBack={handleBack}
      >
      ...
      {/* Basic Info */}
      <FormSection title={t("roles.section.basic")}>
      ...
      {/* Members */}
      <FormSection title={t("roles.section.members")}>
        <FormField label={t("roles.field.users")} tooltip={t("roles.tooltip.users")} span="full">
          <MultiSearchSelect
            selected={role.users || []}
            options={userOptions}
        ...
        <FormField label={t("roles.field.groups")} tooltip={t("roles.tooltip.groups")} span="full">
          <MultiSearchSelect
        ...
        <FormField label={t("roles.field.roles")} tooltip={t("roles.tooltip.roles")} span="full">
          <MultiSearchSelect
            selected={role.roles || []}
            options={roleOptions}
        ...
        <FormField label={t("roles.field.domains")} tooltip={t("roles.tooltip.domains")} span="full">
          <MultiSearchSelect
            selected={role.domains || []}
            options={(role.domains || []).map((d) => ({ value: d, label: d }))}
            onChange={(v) => set("domains", v)}
            placeholder={t("roles.field.domainsPlaceholder")}
          />
        </FormField>
      </FormSection>

      {/* Options */}
      <FormSection title={t("roles.section.options")}>
        <FormField label={t("col.isEnabled")} tooltip={t("roles.tooltip.isEnabled")}>
