...
export default function PlanListPage() {
  ...
  const prefs = useTablePrefs({ persistKey: "list:plans" });
  const bulkDelete = useBulkDelete<Plan>(PlanBackend.deletePlan, list.refetch);
  ...
  const columns: Column<Plan>[] = [
    ...
    {
      key: "price", title: t("plans.field.price"), sortable: true, filterable: true, width: "120px",
      render: (_, r) => <span className="text-[12px] font-mono">{r.currency} {r.price}</span>,
    },
    { key: "period", title: t("plans.field.period"), filterable: true, width: "120px" },
    {
      key: "role", title: t("plans.field.role"), filterable: true, width: "140px",
    ...
    {
      key: "product", title: t("plans.field.product"), filterable: true, width: "140px",
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight">{t("plans.title")}</h1>
          <p className="text-[13px] text-text-muted mt-0.5">{t("plans.subtitle")}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleAdd} className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2 text-[13px] font-semibold text-white hover:bg-accent-hover transition-colors"><Plus size={15} /> {t("plans.add")}</button>
        </div>
      </div>
      <DataTable
        columns={columns}
