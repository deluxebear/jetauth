...
export default function PasswordModal({
  ...
  const handleSubmit = async () => {
    if (!newPassword) {
      setError(t("password.error.empty"));
      return;
    }
    ...
    if (needOldPassword && !oldPassword) {
      setError(t("password.error.oldRequired"));
      return;
    }
    ...
    try {
      ...
      if (res.status === "ok") {
        modal.toast(t("password.success"));
        setOpen(false);
      } else {
        setError(res.msg || t("password.error.failed"));
      }
    } catch (e: any) {
      setError(e.message || t("password.error.failed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <button type="button" onClick={handleOpen} disabled={disabled}
        ...
        <KeyRound size={14} />
        {hasExistingPassword ? t("password.modify") : t("password.set")}
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md rounded-xl border border-border bg-surface-1 shadow-[var(--shadow-elevated)] p-6 space-y-4">
            <h2 className="text-[16px] font-bold text-text-primary">{t("password.title")}</h2>
            ...
            {needOldPassword && (
              <div>
                <label className="block text-[12px] font-medium text-text-secondary mb-1.5">{t("password.old")}</label>
                <div className="relative">
                  <input type={showOld ? "text" : "password"} value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} className={ic} placeholder={t("password.placeholder")} />
            ...
            <div>
              <label className="block text-[12px] font-medium text-text-secondary mb-1.5">{t("password.new")}</label>
              <div className="relative">
                <input type={showNew ? "text" : "password"} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className={ic} placeholder={t("password.placeholder")} />
            ...
            <div>
              <label className="block text-[12px] font-medium text-text-secondary mb-1.5">{t("password.confirm")}</label>
              <div className="relative">
              ...
              {confirmPassword && confirmPassword !== newPassword && (
                <p className="text-[11px] text-danger mt-1">{t("password.error.mismatch")}</p>
              )}
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="button" onClick={handleSubmit} disabled={loading || !newPassword || !confirmPassword || (needOldPassword && !oldPassword)}
                ...
                {loading ? <span className="inline-block h-4 w-4 rounded-full border-2 border-white/30 border-t-white animate-spin" /> : t("password.submit")}
              </button>
            </div>
          </div>
        </div>
