  ...
  const columns: Column<Product>[] = [
    {
      ...
      render: (_, r) => <Link to={`/products/${r.owner}/${encodeURIComponent(r.name)}`} className="font-mono font-medium text-accent hover:underline" onClick={(e) => e.stopPropagation()}>{r.name}</Link>,
    },
    {
    ...
    {
      key: "image", title: t("products.field.image" as any), width: "170px",
      render: (_, r) => r.image ? <a href={r.image} target="_blank" rel="noreferrer"><img src={r.image} alt={r.name} className="h-8 object-contain" /></a> : <span className="text-text-muted">{"\u2014"}</span>,
    },
    { key: "tag", title: t("products.field.tag" as any), sortable: true, filterable: true, width: "160px" },
    {
      key: "price", title: t("products.field.price" as any), sortable: true, filterable: true, width: "120px",
      render: (_, r) => <span className="text-[12px] font-mono">{r.currency} {r.price}</span>,
    },
    {
      key: "quantity", title: t("products.field.quantity" as any), sortable: true, width: "100px",
      render: (_, r) => <span className="text-[12px] text-text-muted">{r.quantity}</span>,
    },
    {
      key: "sold", title: t("products.field.sold" as any), sortable: true, width: "100px",
      render: (_, r) => <span className="text-[12px] text-text-muted">{r.sold}</span>,
    },
    { key: "state", title: t("col.state" as any), sortable: true, filterable: true, width: "120px" },
    {
      key: "providers", title: t("products.field.providers" as any), filterable: true, width: "200px",
      render: (_, r) => <span className="text-[12px] text-text-muted">{r.providers?.length ?? 0} {t("products.field.providers" as any).toLowerCase()}</span>,
    },
    {
      key: "__actions", fixed: "right" as const, title: t("common.action" as any), width: "110px",
      render: (_, r) => (
        <div className="flex items-center gap-1">
          <Link to={`/products/${r.owner}/${encodeURIComponent(r.name)}`} className="rounded p-1.5 text-text-muted hover:text-warning hover:bg-warning/10 transition-colors" title={t("common.edit")} onClick={(e) => e.stopPropagation()}><Pencil size={14} /></Link>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight">{t("products.title" as any)}</h1>
          <p className="text-[13px] text-text-muted mt-0.5">{t("products.subtitle" as any)}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleAdd} className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2 text-[13px] font-semibold text-white hover:bg-accent-hover transition-colors"><Plus size={15} /> {t("products.add" as any)}</button>
        </div>
      </div>
