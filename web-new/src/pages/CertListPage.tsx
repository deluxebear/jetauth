...
export default function CertListPage() {
  ...
  const columns: Column<Cert>[] = [
    ...
    { key: "scope", title: t("col.scope" as any), sortable: true, width: "90px" },
    { key: "type", title: t("col.type" as any), sortable: true, width: "90px" },
    {
      key: "cryptoAlgorithm", title: t("col.algorithm" as any), sortable: true, width: "120px",
      render: (_, r) => <span className="font-mono text-[11px] text-text-muted">{r.cryptoAlgorithm}</span>,
    },
    {
      key: "bitSize", title: t("certs.field.bitSize" as any), sortable: true, width: "90px",
      render: (_, r) => <span className="font-mono text-[11px] text-text-muted">{r.bitSize || "—"}</span>,
    },
    { key: "expireInYears", title: t("certs.field.expireInYears" as any), sortable: true, width: "100px" },
    {
      key: "__actions", fixed: "right" as const, title: t("common.action" as any), width: "140px",
      render: (_, r) => (
        <div className="flex items-center gap-1">
          {r.type === "SSL" && (
            <button onClick={(e) => handleRefreshDomain(r, e)} className="rounded p-1.5 text-text-muted hover:text-info hover:bg-info/10 transition-colors" title={t("certs.refreshDomain" as any)}><RotateCw size={14} /></button>
          )}
          <Link to={`/certs/${r.owner}/${r.name}`} className="rounded p-1.5 text-text-muted hover:text-warning hover:bg-warning/10 transition-colors" title={t("common.edit")} onClick={(e) => e.stopPropagation()}><Pencil size={14} /></Link>
