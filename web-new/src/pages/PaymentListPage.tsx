...
export default function PaymentListPage() {
  ...
  const list = useEntityList<Payment>({
    queryKey: "payments",
    fetchFn: PaymentBackend.getPayments,
  });

  const handleAdd = async () => {
    const payment = PaymentBackend.newPayment(getNewEntityOwner());
    const res = await PaymentBackend.addPayment(payment);
    if (res.status === "ok") {
  ...
  const handleDelete = (record: Payment, e: React.MouseEvent) => {
    e.stopPropagation();
    modal.showConfirm(
  ...
  return (
    <div className="space-y-5">
      ...
      <DataTable columns={columns} data={list.items} rowKey="name" loading={list.loading} page={list.page} pageSize={list.pageSize} total={list.total} onPageChange={list.setPage} onSort={list.handleSort} onFilter={list.handleFilter} emptyText={t("common.noData")} persistKey="list:payments" resizable columnsToggle />
    </div>
  );
}
