...
| File | Responsibility |
|------|---------------|
| `web-new/src/api/client.ts` | **Modify** — Add `postForm()` helper for form-urlencoded POST |
| `web-new/src/pages/MfaSetup.tsx` | **New** — 3-step MFA setup page |
| `web-new/src/pages/MfaVerify.tsx` | **New** — MFA verification page |
| `web-new/src/App.tsx` | **Modify** — Add MFA state, routes, login flow branching |
| `web-new/src/locales/en.ts` | **Modify** — Add MFA page translations |
| `web-new/src/locales/zh.ts` | **Modify** — Add MFA page translations |
...
- Modify: `web-new/src/api/client.ts`

- [ ] **Step 1: Install qrcode package**

```bash
cd /Users/xiongyanlin/projects/jetauth/web-new && npm install qrcode @types/qrcode
```

- [ ] **Step 2: Add postForm helper to api client**

In `web-new/src/api/client.ts`, add after the existing `api` object (after line 24):
...
```bash
git add web-new/package.json web-new/package-lock.json web-new/src/api/client.ts
...
Find the line with `"theme.followSystem"` in `web-new/src/locales/en.ts` and add after it:
...
Find the line with `"theme.followSystem"` in `web-new/src/locales/zh.ts` and add after it:
...
**Files:**
- Create: `web-new/src/pages/MfaSetup.tsx`

- [ ] **Step 1: Create the MFA setup page**

Create `web-new/src/pages/MfaSetup.tsx` with the following complete implementation:
...
```bash
cd /Users/xiongyanlin/projects/jetauth/web-new && npx tsc --noEmit 2>&1 | head -20
```

- [ ] **Step 3: Commit**

```bash
git add web-new/src/pages/MfaSetup.tsx
...
**Files:**
- Create: `web-new/src/pages/MfaVerify.tsx`
...
```bash
git add web-new/src/pages/MfaVerify.tsx
...
At the top of `web-new/src/App.tsx`, add imports for MfaSetup and MfaVerify after the Signup import:
...
```bash
cd /Users/xiongyanlin/projects/jetauth/web-new && npm run dev
```
